...
int datatype::name_c2i(char *name)
{
    ...
    if(!strcmp(name,"dim_displacement"))return dim_displacement;
    if(!strcmp(name,"dim_disp_x"))return dim_disp_x;
    if(!strcmp(name,"dim_disp_z"))return dim_disp_z;
...
void datatype::name_i2c(int ind,char *name)
{
    ...
    if(ind==dim_disp_x){sprintf(name,"dim_disp_x");return;}
    if(ind==dim_disp_z){sprintf(name,"dim_disp_z");return;}
    ...
    void general_data_base::print_all(bool skip_dimens, bool slim)
    {
       double cut=(slim==true)?1e-15:-1.0;
       print_all(cut, skip_dimens);
    }


    void general_data_base::print_all(double cut, bool skip_dimens)
    {
	if(!skip_dimens)
        	print();
